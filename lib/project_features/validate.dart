class Validator {
  ...
  void validateOperatorBeforeParentheses() {
    ...
    for (int i = 1; i < _question.length; i++) {
      if (_question[i] == '(' && !_basicOperators.contains(_question[i - 1]) || _question[i] == 't' && !_basicOperators.contains(_question[i - 1]) || _question[i] == 'n' && !_basicOperators.contains(_question[i - 1]) )  {
